...
// IMPORTANT: Use otakudesu.fit as requested
...
// Convert episode slug to series slug by removing episode suffix
function epSlugToSeriesSlug(epSlug: string): string {
  return epSlug.replace(/-episode-\d+-.*$/i, '').replace(/\/+$/g, '');
}

// Resolve the real series slug by first trying direct URL, then fetching an episode page
async function resolveSeriesSlug(slugOrEp: string): Promise<string | null> {
  // If slug contains "episode", try to extract series from episode page
  if (slugOrEp.includes('episode')) {
    // Try direct: remove episode suffix
    const direct = epSlugToSeriesSlug(slugOrEp);
    const testUrl = `${BASE}/series/${direct}/`;
    const testHtml = await fetchPage(testUrl);
    if (testHtml) {
      const $$ = cheerio.load(testHtml);
      const h1 = $$('h1').first().text().trim();
      // If not redirected to homepage (check for series detail page signature)
      if (h1 && h1.length > 5 && !$$('body').html()?.includes('Latest Release')) {
        return direct;
      }
    }

    // Fallback: fetch episode page and extract series link from breadcrumb
    const epUrl = `${BASE}/${slugOrEp}/`;
    const epHtml = await fetchPage(epUrl);
    if (epHtml) {
      const $$e = cheerio.load(epHtml);
      const seriesLink = $$e('a[href*="/series/"]').first().attr('href') || '';
      if (seriesLink) {
        // Extract slug from the series URL: /series/some-slug/
  ...
  // Not an episode slug — use directly
  return slugOrEp.replace(/\/+$/g, '');
}
...
export async function getDetail(slug: string): Promise<AnimeDetail | null> {
  // Resolve real series slug (handle episode slugs that don't match perfectly)
