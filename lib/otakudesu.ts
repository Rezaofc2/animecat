...
export async function getTerbaru(page = 1): Promise<AnimeCard[]> {
  ...
  $('article, .post, .animepost').each((_: number, el: any) => {
    ...
    const dateEl = $(el).find('.date, .epdate, time');
    const date = dateEl.text().trim() || undefined;
    const typeEl = $(el).find('.type, .status');
    const type = typeEl.text().trim() || undefined;
    if (title && slug) results.push({ title: shortTitle(title), slug, poster, episode, date, type });
  });
  return results;
}
...
export async function getDetail(slug: string): Promise<AnimeDetail | null> {
  ...
  const synopsis = $('.entry-content p, .sinopsis, .desc, .synopsis, .summary p').first().text().trim();
