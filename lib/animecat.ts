...
function getPoster($: any, el: any): string {
  ...
  if (dataSrc && !dataSrc.includes('svg+xml') && !dataSrc.includes('js/jquery')) return dataSrc;
...
async function apiGet(url: string): Promise<string|null> {
...
export async function getDetail(slug: string): Promise<AnimeDetail | null> {
  ...
  const poster = (posterImg.attr('data-src') && !posterImg.attr('data-src')?.includes('svg+xml') ? posterImg.attr('data-src') : posterImg.attr('src')) || '';
  ...
  // Stream URL - check for iframe or video sources
  let streamUrl: string | undefined;
  const iframe = $('iframe').first().attr('src');
  if (iframe && !iframe.includes('facebook.com') && !iframe.includes('twitter.com')) {
    streamUrl = iframe;
  }
  ...
  return {
    ...
    genres, synopsis, episodes, streamUrl,
  };
}
...
export async function getEpisode(slug: string): Promise<EpisodeData | null> {
  ...
  // Stream URL - check for iframe
  let streamUrl: string | undefined;
  const iframe = $('iframe').first().attr('src');
  if (iframe && !iframe.includes('facebook')) streamUrl = iframe;
