...
export async function getTerbaru(page = 1): Promise<AnimeCard[]> {
  ...
  // Filter hanya Ongoing
  return results.filter(r => !r.episode || r.episode.toLowerCase().includes('ongoing') || r.episode.toLowerCase().includes('sub'));
}
...
export async function getEpisode(slug: string): Promise<EpisodeData | null> {
  ...
  const prevHref = $('.prev a, .naveps .prev a, .prevpost a, .naveps .prevpage a').attr('href') || '';
  const nextHref = $('.next a, .naveps .next a, .nextpost a, .naveps .nextpage a').attr('href') || '';
  const prevSlug = prevHref.replace(BASE, '').replace(/^\/+|\/+$/g, '') || undefined;
  const nextSlug = nextHref.replace(BASE, '').replace(/^\/+|\/+$/g, '') || undefined;

  // Find real video URL - check iframe and various embed patterns
  let streamUrl: string | undefined;
  const iframeSrc = $('iframe').first().attr('src') || $('iframe[src*="embed"]').attr('src');
  if (iframeSrc && !iframeSrc.includes('facebook')) streamUrl = iframeSrc;
  ...
  $('a').each((_, el) => {
    ...
    // match download + streaming links
    const isDL = /gofile|krakenfiles|acefile|pixeldrain|mediafire|filedon|vidhide|mp4upload|drive\.google|mega\.nz|streamtape|doodstream|voe\.sx|mixdrop/i.test(href);
    if (!isDL) return;
    foundUrls.add(href);
