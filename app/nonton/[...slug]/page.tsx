...
export default function NontonPage() {
  ...
  // Current stream: blogger embed URL from scraper
  const currentStream = data?.streamUrl || '';

  // Get download link for selected resolution
  const selectedDownloadUrl = selectedResolution
    ? data?.downloads.find(d => d.quality === selectedResolution)?.links[0]?.url
    : data?.downloads[0]?.links[0]?.url;
          ...
          {currentStream ? (
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-500">
              ...
              {data.downloads.length > 0 && (
                <a href={selectedDownloadUrl || '#'} target="_blank" rel="noopener noreferrer" className="mt-3 px-4 py-2 bg-gradient-to-r from-cyan-500 to-blue-600 text-white rounded-full text-sm font-semibold">
            ...
            <div className="max-w-5xl mx-auto px-4 mt-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                {['360p','480p','720p','1080p'].map(res => {
                  const dl = data.downloads.find(d => d.quality.toUpperCase().includes(res));
                  const url = dl?.links[0]?.url || '#';
                  const active = selectedResolution === dl?.quality;
                  return (
                    <button
                      key={res}
                      onClick={() => {
                        setSelectedResolution(dl?.quality || '');
              ...
              {selectedResolution && (
                <p className="text-[9px] text-slate-500 mt-1">Streaming: <span className="text-cyan-400">{selectedResolution}</span> — klik resolusi untuk menonton</p>
              )}
            </div>

            {/* ===== TOMBOL DOWNLOAD OREN ===== */}
            <div className="max-w-5xl mx-auto px-4 mt-4">
              <a
                ...
                {selectedResolution
                  ...
                  : data.downloads[0]
                    ? `Download ${data.downloads[0].quality}`
                    : 'Download Episode'}
              </a>
            </div>
