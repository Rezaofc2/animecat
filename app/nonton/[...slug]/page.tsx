  ...
  return (
    <div className={'min-h-screen bg-[#0a0a12] ' + (fullscreen ? 'bg-black' : '')}>
      {!fullscreen && (
        <header className="sticky top-0 z-50 bg-[#0a0a12]/95 backdrop-blur border-b border-white/[0.04]">
          <div className="max-w-5xl mx-auto px-4 h-14 flex items-center gap-2">
            ...
            <button onClick={toggleFullscreen} className="p-2 text-slate-400 hover:text-cyan-400 transition-colors rounded-lg hover:bg-white/[0.05]" title="Fullscreen"><Maximize size={16} /></button>
          </div>
        </header>
      )}
      ...
      <div ref={containerRef} className={fullscreen ? 'fixed inset-0 z-[9999] bg-black flex flex-col' : ''}>
        ...
        <div className={'relative bg-black flex items-center justify-center ' + (fullscreen ? 'flex-1' : 'aspect-video max-w-5xl mx-auto mt-4 rounded-xl overflow-hidden border border-white/[0.06]')}>
          <div className="text-center text-slate-500">
            <Play size={fullscreen ? 64 : 48} className="mx-auto mb-3 text-slate-600" />
            <p className="text-sm sm:text-base">🎬 Streaming dari Samehadaku</p>
            <p className="text-[11px] sm:text-xs mt-1 text-slate-600">Klik Download untuk menonton &amp; download</p>
            {!fullscreen && (
              <button onClick={toggleFullscreen} className="mt-4 px-4 py-2 bg-white/[0.05] border border-white/[0.08] rounded-full text-xs text-slate-300 hover:bg-white/[0.08] transition-colors inline-flex items-center gap-1.5">
                <Maximize size={13} />Fullscreen
              </button>
            )}
          </div>
          {!fullscreen && (
            <button onClick={toggleFullscreen} className="absolute top-3 right-3 p-2 bg-black/50 hover:bg-black/70 rounded-lg text-white transition-colors z-10" title="Fullscreen"><Maximize size={16} /></button>
          )}
        </div>
          ...
          <>
            <div className="max-w-5xl mx-auto px-4 mt-4">
              <div className="grid grid-cols-3 gap-2">
                ...
                <div>
                  {data.nextSlug ? <Link href={'/nonton/'+data.nextSlug} className="flex items-center justify-center gap-1 px-3 py-2.5 bg-gradient-to-r from-cyan-500 to-blue-600 text-white rounded-xl text-xs font-semibold hover:shadow-lg hover:shadow-cyan-500/25 transition-all">Berikutnya<ChevronRight size={14} /></Link> : <span className="flex items-center justify-center gap-1 px-3 py-2.5 text-xs text-slate-700">—</span>}
                </div>
              </div>
            </div>
              ...
              <div className="max-w-5xl mx-auto px-4 mt-6 mb-12">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-1.5">
                  {data.relatedEpisodes.slice(0,20).map((ep,i) => (
                    <Link key={i} href={'/nonton/'+ep.slug} className={'px-3 py-2 bg-white/[0.02] border border-white/[0.04] rounded-lg text-xs transition-all flex items-center justify-between '+(ep.slug===slug?'border-cyan-400/30 bg-cyan-500/10 text-cyan-300':'text-slate-400 hover:bg-white/[0.05] hover:text-slate-200')}>
                      <span className="truncate">{ep.title}</span>
                      {ep.date && <span className="text-[9px] text-slate-600 ml-2 shrink-0">{ep.date}</span>}
                    </Link>
                  ))}
                </div>
              </div>
