...
export default function NontonPage() {
  ...
  // Try to extract direct embed URL from samehadaku if iframe doesn't work
  const buildEmbedSrc = () => {
    if (data?.streamUrl && data.streamUrl.includes('blogger.com/video.g')) return data.streamUrl;
    // Try samehadaku embedded page
    return `https://samehadaku.li/${slug}/`;
  };
          ...
          {data.streamUrl ? (
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-500">
              ...
              {data.downloads.length > 0 && (
                <a href={data.downloads[0].links[0]?.url || '#'} target="_blank" rel="noopener noreferrer" className="mt-3 px-4 py-2 bg-gradient-to-r from-cyan-500 to-blue-600 text-white rounded-full text-sm font-semibold hover:shadow-lg">
                  <Download size={14} className="inline mr-1" />Download & Tonton
                </a>
              )}
            </div>
          )}
            ...
            {data.downloads.length > 0 && (
              <div className="max-w-5xl mx-auto px-4 mt-3">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-[11px] text-slate-500 font-medium">📥 Download:</span>
                  {data.downloads.map((dl, i) => (
                    <a key={i} href={dl.links[0]?.url || '#'} target="_blank" rel="noopener noreferrer" className="px-3 py-1.5 bg-white/[0.04] border border-white/[0.08] rounded-full text-[11px] font-semibold text-slate-300 hover:bg-cyan-500/15 hover:text-cyan-300 hover:border-cyan-400/30 transition-all flex items-center gap-1">
                      {dl.quality} <ExternalLink size={9} />
                    </a>
                  ))}
                </div>
              </div>
            ...
            <div className="max-w-5xl mx-auto px-4 mt-4 mb-8">
              <details className="group">
                ...
                <div className="mt-3 bg-white/[0.02] border border-white/[0.06] rounded-xl p-4">
                  ...
                  ) : <p className="text-xs text-slate-500 text-center py-2">Tidak ada link download. Buka langsung di <a href={'https://samehadaku.li/'+slug+'/'} target="_blank" className="text-cyan-400">Samehadaku</a>.</p>}
              ...
              <div className="max-w-5xl mx-auto px-4 mb-12">
                ...
                <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                  {data.relatedEpisodes.slice(0, 20).map((ep,i) => (
                    <Link key={i} href={'/nonton/'+ep.slug} className={'px-3 py-2 border rounded-lg text-xs transition-all flex items-center gap-1 shrink-0 '+(ep.slug===slug?'border-cyan-400/30 bg-cyan-500/10 text-cyan-300':'border-white/[0.04] bg-white/[0.01] text-slate-400 hover:bg-white/[0.05] hover:text-slate-200')}>
                      <span>{shortEp(ep.title)}</span>
                    </Link>
                  ))}
                </div>
              </div>
