...
import { ArrowLeft, Play, Tv, Circle, ChevronRight } from "lucide-react";
...
export default async function AnimeDetailPage({ params }: { params: Promise<{ slug: string[] }> }) {
  ...
  const detail = await apiGet<AnimeDetail>('/api/animecat?action=detail&slug=' + encodeURIComponent(slug));
  if (!detail) notFound();
          ...
          {detail.episodes.length>0 ? (
            <div className="space-y-1.5">
              {detail.episodes.map((ep,i) => (
                <Link key={i} href={'/nonton/'+ep.slug} className="flex items-center gap-3 px-4 py-2.5 bg-white/[0.02] border border-white/[0.04] rounded-xl hover:bg-white/[0.05] hover:border-cyan-400/20 transition-all group">
                  <span className="text-xs font-bold text-cyan-400 w-16 shrink-0">{ep.title}</span>
                  <span className="flex-1 text-sm text-slate-300 group-hover:text-white transition-colors truncate">{ep.title}</span>
                  {ep.date && <span className="text-[10px] text-slate-500 shrink-0">{ep.date}</span>}
