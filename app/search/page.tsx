...
function SearchPageInner() {
  ...
  const doSearch = useCallback(async () => {
    ...
    if (tab === "cari" && query.trim()) {
      const url = '/api/animecat?action=search&q=' + encodeURIComponent(query) + '&page=' + page;
      try { const res = await fetch(url); const d = await res.json(); allResults = Array.isArray(d)?d:[]; } catch { allResults = []; }
    } else if (tab === "genre" && genres.length > 0) {
      // Fetch each genre page, merge & dedup by slug
      const allResultsMap = new Map<string, AnimeCard>();
      const genreUrls = genres.map(g => '/api/animecat?action=daftar&genre=' + encodeURIComponent(g) + '&page=' + page);
      try {
        const responses = await Promise.allSettled(genreUrls.map(u => fetch(u).then(r => r.json())));
        for (const r of responses) {
          if (r.status === 'fulfilled' && Array.isArray(r.value)) {
            for (const item of r.value) {
              if (!allResultsMap.has(item.slug)) allResultsMap.set(item.slug, item);
            }
          }
        }
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 w-full py-6">
        ...
        {results.length>0 && (
          <div className="flex items-center justify-center gap-3 mt-8">
            <button onClick={()=>setPage(p=>Math.max(1,p-1))} disabled={page===1} className="p-2.5 rounded-lg bg-white/[0.04] border border-white/[0.06] text-slate-300 hover:bg-white/[0.08] disabled:opacity-30 disabled:cursor-not-allowed transition-all hover:scale-105 active:scale-95"><ChevronLeft size={18}/></button>
            <span className="text-xs text-slate-400 px-2 font-medium">Hal. {page}</span>
            <button onClick={()=>setPage(p=>p+1)} disabled={results.length===0} className="p-2.5 rounded-lg bg-white/[0.04] border border-white/[0.06] text-slate-300 hover:bg-white/[0.08] disabled:opacity-30 disabled:cursor-not-allowed transition-all hover:scale-105 active:scale-95"><ChevronRight size={18}/></button>
          </div>
        )}
      </main>
