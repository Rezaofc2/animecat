...
function SearchPageInner() {
  ...
  const doSearch = useCallback(async () => {
    ...
    if (tab === "cari" && query.trim()) {
      ...
      // Fetch each genre and merge, dedup by slug
      const genreUrls = genres.map(g => '/api/animecat?action=daftar&genre=' + encodeURIComponent(g) + '&order=' + order + '&page=' + page);
      const responses = await Promise.allSettled(genreUrls.map(u => fetch(u).then(r => r.json())));
      const seen = new Set<string>();
      for (const r of responses) {
        if (r.status === 'fulfilled' && Array.isArray(r.value)) {
          for (const item of r.value) {
            if (!seen.has(item.slug)) { seen.add(item.slug); allResults.push(item); }
          }
        }
      }
    } else if (tab === "terbaru") {
