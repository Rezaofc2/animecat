      ...
      <header className="sticky top-0 z-50 bg-[#0a0a12]/95 backdrop-blur border-b border-white/[0.04]">
        ...
        <div className="max-w-7xl mx-auto px-4 flex items-center gap-1 overflow-x-auto pb-2">
          ...
          {(tab==="daftar"||tab==="genre"||tab==="terbaru") && (
            <select value={order} onChange={e=>{setOrder(e.target.value);setPage(1);}} className="ml-auto px-3 py-1.5 bg-white/[0.04] border border-white/[0.08] rounded-full text-[11px] text-slate-300 focus:outline-none">
          ...
          {tab==="genre" && (
            <button onClick={()=>setShowGenrePicker(d=>!d)}
              className="ml-1 px-3 py-1.5 rounded-full text-[11px] font-semibold transition-all flex items-center gap-1 bg-cyan-500/10 text-cyan-400 border border-cyan-400/30">
              Pilih Genre <ChevronDown size={12} className={showGenrePicker?'rotate-180':''} />
            </button>
          )}
        </div>
        {/* Genre dropdown — always visible when tab=genre */}
        ...
        {tab==="genre" && genre && (
          <div className="max-w-7xl mx-auto px-4 pb-2 flex items-center gap-2">
            <span className="text-[11px] text-slate-500">Genre:</span>
            <span className="px-2.5 py-1 bg-cyan-500/15 text-cyan-400 border border-cyan-400/30 rounded-full text-[10px] font-semibold flex items-center gap-1 capitalize">
              {genre.replace(/-/g,' ')}
              <button onClick={()=>{setGenre('');setPage(1);}}><X size={10} /></button>
            </span>
          </div>
        )}
      </header>
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 w-full py-6">
        {(tab==="cari"||tab==="terbaru"||tab==="daftar"||tab==="genre") && (
          <>
            ...
            {tab==="genre" && !genre && <div className="text-center py-10 text-slate-500"><p>Pilih genre di atas untuk memfilter anime</p></div>}
