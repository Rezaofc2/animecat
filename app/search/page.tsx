...
function SearchPageInner() {
  ...
  const doSearch = useCallback(async () => {
    ...
    if (tab === "cari" && query.trim()) {
      ...
      // Fetch up to 5 pages & dedup — samehadaku genre pages return same data, so show all without pagination
      const all = new Map<string, AnimeCard>();
      try {
        const pages = [1,2,3,4,5];
        const responses = await Promise.allSettled(pages.map(p => fetch('/api/animecat?action=daftar&genre=' + encodeURIComponent(genre) + '&page=' + p).then(r=>r.json())));
        for (const r of responses) {
          if (r.status === 'fulfilled' && Array.isArray(r.value)) {
            for (const item of r.value) { if (!all.has(item.slug)) all.set(item.slug, item); }
          }
        }
      } catch(e) {}
      allResults = Array.from(all.values());
    } else if (tab === "terbaru") {
    ...
    else if (tab==="genre" && genre) { p.set("genre",genre); }
