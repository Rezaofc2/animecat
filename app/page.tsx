...
import { Play, Sparkles, Flame, Search, Tv, Film, Server, Clock, Calendar } from "lucide-react";

interface AnimeCard { title: string; slug: string; poster: string; episode?: string; rating?: string; type?: string; date?: string; day?: string; episodeSlug?: string; }
  ...
  return (
    <div className="min-h-screen bg-[#0a0a12] flex flex-col">
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 w-full space-y-8 pb-12">
          ...
          {loading ? (
            ...
            <div className="flex gap-3 overflow-x-auto pb-3 scrollbar-hide -mx-1 px-1" style={{scrollSnapType:'x mandatory'}}>
              {terbaru.slice(0,16).map((item,i) => {
                ...
                return (
                <Link key={i} href={'/anime/'+item.slug+'?server='+server} className="w-40 sm:w-44 shrink-0 block group" style={{scrollSnapAlign:'start'}}>
                  <div className="relative aspect-[3/4] rounded-xl overflow-hidden bg-white/[0.03] border border-white/[0.06]">
                    ...
                    {/* Release date & day - bottom overlay */}
                    {item.date && (
        ...
        {/* Populer list - only for server 1 (Samehadaku) */}
        {server === 1 && (
        <section>
