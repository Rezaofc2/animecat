    ...
    <div className="min-h-screen bg-[#0a0a12] flex flex-col">
      <header className="sticky top-0 z-50 bg-[#0a0a12]/95 backdrop-blur border-b border-white/[0.04]">
        <div className="max-w-7xl mx-auto px-4 h-14 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-3 text-sm text-slate-400">
            ...
            <Link href="/search?tab=daftar" className="hover:text-cyan-300">A-Z</Link>
          </nav>
        </div>
      </header>
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 w-full space-y-8 pb-12">
        ...
        <section>
          ...
          {loading ? (
            <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide -mx-1 px-1">
              {Array.from({length:8}).map((_,i) => <div key={i} className="w-36 sm:w-40 shrink-0 aspect-[3/4] rounded-xl bg-white/[0.03] border border-white/[0.06] animate-pulse" />)}
            </div>
          ) : terbaru.length > 0 ? (
            <div className="flex gap-3 overflow-x-auto pb-3 scrollbar-hide -mx-1 px-1" style={{scrollSnapType:'x mandatory'}}>
              {terbaru.slice(0,16).map((item,i) => (
                <Link key={i} href={'/anime/'+item.slug} className="w-36 sm:w-40 shrink-0 block group" style={{scrollSnapAlign:'start'}}>
                  <div className="relative aspect-[3/4] rounded-xl overflow-hidden bg-white/[0.03] border border-white/[0.06]">
                    ...
                    {item.episode && <span className="absolute top-1.5 left-1.5 px-1.5 py-0.5 bg-cyan-500/90 text-[9px] font-bold text-white rounded-md">{item.episode}</span>}
                    {item.rating && <span className="absolute top-1.5 right-1.5 px-1 py-0.5 bg-yellow-500/90 text-[9px] font-bold text-black rounded">★ {item.rating}</span>}
                  ...
                  <h3 className="mt-1.5 text-xs font-medium text-slate-200 line-clamp-2 group-hover:text-cyan-300 transition-colors leading-tight">{item.title}</h3>
                </Link>
              ))}
            </div>
        ...
        <section>
          ...
          <div id="history-container" className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide -mx-1 px-1">
            <div className="w-36 shrink-0 text-center py-8 text-slate-500 bg-white/[0.02] rounded-xl text-xs">
              <Clock size={24} className="mx-auto mb-1 text-slate-600" />
